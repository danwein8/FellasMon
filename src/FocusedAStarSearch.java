 ...
 *
 */
public class FocusedAStarSearch {
	 ...
	 */
	protected List constructPath(SubNode node) {
		LinkedList path = new LinkedList();
		while (node.pathParent != null) {
			path.addFirst(node);
			node = (SubNode)node.pathParent;
		}
		return path;
	}
	 ...
	 * AStarNode class
	 */
	public List findPath(SubNode startNode, SubNode goalNode) {
	...
	public List findAttackPath(SubNode startNode, SubNode goalNode) {
		PriorityList openList = new PriorityList();
		LinkedList closedList = new LinkedList();
		...
		while (!openList.isEmpty()) {
			SubNode node = (SubNode)openList.removeFirst();
			// if we've found the goal
			if (node == goalNode) {
				// construct the path from start to goal
				return constructPath(goalNode);
			...
			// otherwise keep looking for the goal
			// get the neighbors of the current node
			List neighbors = node.getAttackNeighbors();
			// check if the neighbor nodes are in the open list or the closed list
			for (int i = 0; i < neighbors.size(); i++) {
				SubNode neighborNode = (SubNode)neighbors.get(i);
				boolean isOpen = openList.contains(neighborNode);
				boolean isClosed = closedList.contains(neighborNode);
				// calculate their cost from start by adding the cost of the neighbor node
				// and the cost from start of the node before the neighbor node
	...
	public List findPokemonPath(SubNode startNode, SubNode goalNode) {
		PriorityList openList = new PriorityList();
		LinkedList closedList = new LinkedList();
		...
		while (!openList.isEmpty()) {
			...
			// otherwise keep looking for the goal
			// get the neighbors of the current node
			List neighbors = node.getPokemonNeighbors();
			// check if the neighbor nodes are in the open list or the closed list
			for (int i = 0; i < neighbors.size(); i++) {
				SubNode neighborNode = (SubNode)neighbors.get(i);
				boolean isOpen = openList.contains(neighborNode);
	...
	public List findItemPath(SubNode startNode, SubNode goalNode) {
		PriorityList openList = new PriorityList();
		LinkedList closedList = new LinkedList();
		...
		while (!openList.isEmpty()) {
			...
			// otherwise keep looking for the goal
			// get the neighbors of the current node
			List neighbors = node.getItemNeighbors();
			// check if the neighbor nodes are in the open list or the closed list
			for (int i = 0; i < neighbors.size(); i++) {
				SubNode neighborNode = (SubNode)neighbors.get(i);
				boolean isOpen = openList.contains(neighborNode);
